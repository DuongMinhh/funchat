...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    private OAuth2User processOAuth2User(OAuth2UserRequest oAuth2UserRequest, OAuth2User oAuth2User) {
        ...
        if(userOptional.isPresent()) {
            ...
            if(!user.getProvider().equals(AuthProvider.of(oAuth2UserRequest.getClientRegistration().getRegistrationId()).name())) {
    ...
    private User registerNewUser(OAuth2UserRequest oAuth2UserRequest, OAuth2UserInfo oAuth2UserInfo) {
        ...
        user.setProvider(AuthProvider.of(oAuth2UserRequest.getClientRegistration().getRegistrationId()).name());
        user.setProviderId(oAuth2UserInfo.getId());
        user.setName(oAuth2UserInfo.getName());
        user.setEmail(oAuth2UserInfo.getEmail());
        user.setPhotoUrl(oAuth2UserInfo.getImageUrl());
        user.setRole(roleRepository.findByName(UserRole.ROLE_USER.name()));    /* Default role of oauth2 user */
